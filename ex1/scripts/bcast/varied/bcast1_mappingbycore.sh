...
echo "BROADCAST 1"
echo "Processes,Size,Latency" > bcast1_core_mapping_genoa.csv
...
do
    ...
    do
        ...
        # Perform osu_bcast
        result_bcast=$(mpirun --map-by core -np $processes --mca coll_tuned_use_dynamic_rules false --mca coll_tuned_bcast_algorithm 1 $OSUBENCH -m $size:$size -x $repetitions -i $repetitions | tee /dev/stderr | awk '!/^#/ { last = $2 } END { print last }')

        echo "$processes, $size, $result_bcast"
        # Write results on CSV
        echo "$processes,$size,$result_bcast" >> bcast1_core_mapping_genoa.csv
    done
done
